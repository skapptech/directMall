...
public class NearSellerViewMode extends ViewModel {
    ...
    public MutableLiveData<NearBySellerMainModel> getSellerProductListRequest(PaginationModel paginationModel) {
        RestClient.getInstance().getService().GetNearBySellerFilter(paginationModel).enqueue(new Callback<NearBySellerMainModel>() {
            @Override
